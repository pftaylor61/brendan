<?php
/**
 * Brendan functions and definitions
 *
 * @package Brendan
 * @since Brendan 0.0.1
 */
 ...
 * @return void
 */
if ( ! function_exists( 'brendan_setup' ) ) {
	function brendan_setup() {
		 ...
		 * If you're building a theme based on Brendan, use a find and replace
		 * to change 'brendan' to the name of your theme in all the template files
		 */
		load_theme_textdomain( 'brendan', trailingslashit( get_template_directory() ) . 'languages' );
		...
		register_nav_menus( array(
				'primary' => esc_html__( 'Primary Menu', 'brendan' )
			) );
		...
		if( brendan_is_woocommerce_active() && !of_get_option( 'woocommerce_breadcrumbs', '1' ) ) {
			add_action( 'init', 'brendan_remove_woocommerce_breadcrumbs' );
		}
	}
}
...
if ( ! function_exists( '_wp_render_title_tag' ) ) {
	function brendan_slug_render_title() { ?>
		<title><?php wp_title( '|', true, 'right' ); ?></title>
	<?php }
	add_action( 'wp_head', 'brendan_slug_render_title' );
}
...
function brendan_fonts_url() {
	 ...
	 */
	$tenor_sans = _x( 'on', 'Tenor Sans font: on or off', 'brendan' );
	 ...
	 * Do not translate into your own language.
	 */
	$subset = _x( 'no-subset', 'Tenor Sans font: add new subset (cyrillic)', 'brendan' );
	 ...
	 * Do not translate into your own language.
	 */
	$kreon = _x( 'on', 'Kreon font: on or off', 'brendan' );
        $irishgrover = _x( 'on', 'Irish Grover font: on or off', 'brendan' );

	if ( 'off' !== $tenor_sans || 'off' !== $irishgrover ) {
		$font_families = array();
		...
		if ( 'off' !== $irishgrover )
			$font_families[] = 'Irish+Grover:400';
 ...
 */
function brendan_mce_css( $mce_css ) {
	$fonts_url = brendan_fonts_url();
...
add_filter( 'mce_css', 'brendan_mce_css' );
...
function brendan_widgets_init() {
	register_sidebar( array(
			'name' => esc_html__( 'Main Sidebar', 'brendan' ),
			'id' => 'sidebar-main',
			'description' => esc_html__( 'Appears in the sidebar on posts and pages except the optional Front Page template, which has its own widgets', 'brendan' ),
	...
	register_sidebar( array(
			'name' => esc_html__( 'Blog Sidebar', 'brendan' ),
			'id' => 'sidebar-blog',
			'description' => esc_html__( 'Appears in the sidebar on the blog and archive pages only', 'brendan' ),
			'before_widget' => '<aside id="%1$s" class="widget %2$s">',
			'after_widget' => '</aside>',
			'before_title' => '<h3 class="widget-title">',
			'after_title' => '</h3>'
	...
	register_sidebar( array(
			'name' => esc_html__( 'Single Post Sidebar', 'brendan' ),
			'id' => 'sidebar-single',
			'description' => esc_html__( 'Appears in the sidebar on single posts only', 'brendan' ),
			'before_widget' => '<aside id="%1$s" class="widget %2$s">',
			'after_widget' => '</aside>',
			'before_title' => '<h3 class="widget-title">',
			'after_title' => '</h3>'
	...
	register_sidebar( array(
			'name' => esc_html__( 'Page Sidebar', 'brendan' ),
			'id' => 'sidebar-page',
			'description' => esc_html__( 'Appears in the sidebar on pages only', 'brendan' ),
			'before_widget' => '<aside id="%1$s" class="widget %2$s">',
			'after_widget' => '</aside>',
			'before_title' => '<h3 class="widget-title">',
			'after_title' => '</h3>'
	...
	register_sidebar( array(
			'name' => esc_html__( 'First Front Page Banner Widget', 'brendan' ),
			'id' => 'frontpage-banner1',
			'description' => esc_html__( 'Appears in the banner area on the Front Page', 'brendan' ),
	...
	register_sidebar( array(
			'name' => esc_html__( 'Second Front Page Banner Widget', 'brendan' ),
	...
	register_sidebar( array(
			'name' => esc_html__( 'First Front Page Widget Area', 'brendan' ),
			'id' => 'sidebar-homepage1',
			'description' => esc_html__( 'Appears when using the optional Front Page template with a page set as Static Front Page', 'brendan' ),
			'before_widget' => '<aside id="%1$s" class="widget %2$s">',
			'after_widget' => '</aside>',
			'before_title' => '<h3 class="widget-title">',
			'after_title' => '</h3>'
	...
	register_sidebar( array(
			'name' => esc_html__( 'Second Front Page Widget Area', 'brendan' ),
	...
	register_sidebar( array(
			'name' => esc_html__( 'Third Front Page Widget Area', 'brendan' ),
	...
	register_sidebar( array(
			'name' => esc_html__( 'Fourth Front Page Widget Area', 'brendan' ),
	...
	register_sidebar( array(
			'name' => esc_html__( 'First Footer Widget Area', 'brendan' ),
			'id' => 'sidebar-footer1',
			'description' => esc_html__( 'Appears in the footer sidebar', 'brendan' ),
			'before_widget' => '<aside id="%1$s" class="widget %2$s">',
			'after_widget' => '</aside>',
			'before_title' => '<h3 class="widget-title">',
			'after_title' => '</h3>'
	...
	register_sidebar( array(
			'name' => esc_html__( 'Second Footer Widget Area', 'brendan' ),
			'id' => 'sidebar-footer2',
			'description' => esc_html__( 'Appears in the footer sidebar', 'brendan' ),
			'before_widget' => '<aside id="%1$s" class="widget %2$s">',
			'after_widget' => '</aside>',
	...
	register_sidebar( array(
			'name' => esc_html__( 'Third Footer Widget Area', 'brendan' ),
			'id' => 'sidebar-footer3',
			'description' => esc_html__( 'Appears in the footer sidebar', 'brendan' ),
			'before_widget' => '<aside id="%1$s" class="widget %2$s">',
			'after_widget' => '</aside>',
	...
	register_sidebar( array(
			'name' => esc_html__( 'Fourth Footer Widget Area', 'brendan' ),
			'id' => 'sidebar-footer4',
			'description' => esc_html__( 'Appears in the footer sidebar', 'brendan' ),
			'before_widget' => '<aside id="%1$s" class="widget %2$s">',
			'after_widget' => '</aside>',
...
function brendan_scripts_styles() {
	 ...
	 *     wp_dequeue_style( 'brendan-fonts' );
	 * }
	 * add_action( 'wp_enqueue_scripts', 'mytheme_dequeue_fonts', 11 );
	 */
	$fonts_url = brendan_fonts_url();
	if ( !empty( $fonts_url ) ) {
		wp_enqueue_style( 'brendan-fonts', esc_url_raw( $fonts_url ), array(), null );
	}
	...
	if ( is_singular() && comments_open() ) {
		...
		wp_localize_script( 'commentvalidate', 'comments_object', array(
			...
			'author'  => esc_html__( 'Please enter your name', 'brendan' ),
			'email'  => esc_html__( 'Please enter a valid email address', 'brendan' ),
			'comment' => esc_html__( 'Please add a comment', 'brendan' ) )
		);
	}
 ...
 * @return void
 */
if ( ! function_exists( 'brendan_content_nav' ) ) {
	function brendan_content_nav( $nav_id ) {
		global $wp_query;
		...
		<nav role="navigation" id="<?php echo $nav_id; ?>" class="<?php echo $nav_class; ?>">
			<h3 class="assistive-text"><?php esc_html_e( 'Post navigation', 'brendan' ); ?></h3>

			<?php if ( is_single() ) { // navigation links for single posts ?>

				<?php previous_post_link( '<div class="nav-previous">%link</div>', '<span class="meta-nav">' . _x( '<i class="fa fa-angle-left"></i>', 'Previous post link', 'brendan' ) . '</span> %title' ); ?>
				<?php next_post_link( '<div class="nav-next">%link</div>', '%title <span class="meta-nav">' . _x( '<i class="fa fa-angle-right"></i>', 'Next post link', 'brendan' ) . '</span>' ); ?>
			...
			elseif ( $wp_query->max_num_pages > 1 && ( is_home() || is_archive() || is_search() ) ) { // navigation links for home, archive, and search pages ?>
				...
				<?php echo paginate_links( array(
					...
					'type' => 'list',
					'prev_text' => wp_kses( __( '<i class="fa fa-angle-left"></i> Previous', 'brendan' ), array( 'i' => array( 
						'class' => array() ) ) ),
					'next_text' => wp_kses( __( 'Next <i class="fa fa-angle-right"></i>', 'brendan' ), array( 'i' => array( 
						'class' => array() ) ) )
				) ); ?>
 ...
 * simply create your own brendan_comment(), and that function will be used instead.
 *
 ...
 * @return void
 */
if ( ! function_exists( 'brendan_comment' ) ) {
		...
		case 'trackback' :
			...
			<li <?php comment_class(); ?> id="comment-<?php comment_ID(); ?>">
				<article id="comment-<?php comment_ID(); ?>" class="pingback">
					<p><?php esc_html_e( 'Pingback:', 'brendan' ); ?> <?php comment_author_link(); ?> <?php edit_comment_link( esc_html__( '(Edit)', 'brendan' ), '<span class="edit-link">', '</span>' ); ?></p>
				...
				<article id="comment-<?php comment_ID(); ?>" class="comment">
					<header class="comment-meta comment-author vcard">
						...
						printf( '<cite class="fn">%1$s %2$s</cite>',
							...
							( $comment->user_id === $post->post_author ) ? '<span> ' . esc_html__( 'Post author', 'brendan' ) . '</span>' : '' );
						printf( '<a href="%1$s" title="Posted %2$s"><time itemprop="datePublished" datetime="%3$s">%4$s</time></a>',
							esc_url( get_comment_link( $comment->comment_ID ) ),
							sprintf( esc_html__( '%1$s @ %2$s', 'brendan' ), esc_html( get_comment_date() ), esc_attr( get_comment_time() ) ),
							get_comment_time( 'c' ),
							/* Translators: 1: date, 2: time */
							sprintf( esc_html__( '%1$s at %2$s', 'brendan' ), get_comment_date(), get_comment_time() )
						);
						?>
					</header> <!-- .comment-meta -->

					<?php if ( '0' == $comment->comment_approved ) { ?>
						<p class="comment-awaiting-moderation"><?php esc_html_e( 'Your comment is awaiting moderation.', 'brendan' ); ?></p>
					<?php } ?>

					<section class="comment-content comment">
						<?php comment_text(); ?>
						<?php edit_comment_link( esc_html__( 'Edit', 'brendan' ), '<p class="edit-link">', '</p>' ); ?>
					</section> <!-- .comment-content -->

					<div class="reply">
						<?php comment_reply_link( array_merge( $args, array( 'reply_text' => wp_kses( __( 'Reply <span>&darr;</span>', 'brendan' ), array( 'span' => array() ) ), 'depth' => $depth, 'max_depth' => $args['max_depth'] ) ) ); ?>
					</div> <!-- .reply -->
				</article> <!-- #comment-## -->
			<?php
			break;
 ...
 */
function brendan_comment_form_default_fields( $fields ) {

	$commenter = wp_get_current_commenter();
	$req = get_option( 'require_name_email' );
	$aria_req = ( $req ? ' aria-required="true"' : "" );

	$fields[ 'author' ] = '<p class="comment-form-author">' . '<label for="author">' . esc_html__( 'Name', 'brendan' ) . ( $req ? ' <span class="required">*</span>' : '' ) . '</label> ' . '<input id="author" name="author" type="text" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30"' . $aria_req . ' /></p>';

	$fields[ 'email' ] =  '<p class="comment-form-email"><label for="email">' . esc_html__( 'Email', 'brendan' ) . ( $req ? ' <span class="required">*</span>' : '' ) . '</label> ' . '<input id="email" name="email" type="text" value="' . esc_attr(  $commenter['comment_author_email'] ) . '" size="30"' . $aria_req . ' /></p>';

	$fields[ 'url' ] =  '<p class="comment-form-url"><label for="url">' . esc_html__( 'Website', 'brendan' ) . '</label>' . '<input id="url" name="url" type="text" value="' . esc_attr( $commenter['comment_author_url'] ) . '" size="30" /></p>';

	return $fields;

}
add_action( 'comment_form_default_fields', 'brendan_comment_form_default_fields' );
 ...
 */
function brendan_comment_form_field_comment( $field ) {

	$field = '<p class="comment-form-comment"><label for="comment">' . _x( 'Comment', 'noun', 'brendan' ) . ' <span class="required">*</span></label><textarea id="comment" name="comment" cols="45" rows="8" aria-required="true"></textarea></p>';

	return $field;

}
add_action( 'comment_form_field_comment', 'brendan_comment_form_field_comment' );
 ...
 * @return void
 */
if ( ! function_exists( 'brendan_posted_on' ) ) {
	function brendan_posted_on() {
		$post_icon = '';
		...
		$date = sprintf( '<i class="fa %1$s"></i> <a href="%2$s" title="Posted %3$s" rel="bookmark"><time class="entry-date" datetime="%4$s" itemprop="datePublished">%5$s</time></a>',
			...
			sprintf( esc_html__( '%1$s @ %2$s', 'brendan' ), esc_html( get_the_date() ), esc_attr( get_the_time() ) ),
		...
		$author = sprintf( '<i class="fa fa-pencil"></i> <address class="author vcard"><a class="url fn n" href="%1$s" title="%2$s" rel="author">%3$s</a></address>',
			...
			esc_attr( sprintf( esc_html__( 'View all posts by %s', 'brendan' ), get_the_author() ) ),
			get_the_author()
		);

		// Return the Categories as a list
		$categories_list = get_the_category_list( esc_html__( ' ', 'brendan' ) );

		// Translators: 1: Permalink 2: Title 3: No. of Comments
		$comments = sprintf( '<span class="comments-link"><i class="fa fa-comment"></i> <a href="%1$s" title="%2$s">%3$s</a></span>',
			esc_url( get_comments_link() ),
			esc_attr( esc_html__( 'Comment on ' , 'brendan' ) . the_title_attribute( 'echo=0' ) ),
			( get_comments_number() > 0 ? sprintf( _n( '%1$s Comment', '%1$s Comments', get_comments_number(), 'brendan' ), get_comments_number() ) : esc_html__( 'No Comments', 'brendan' ) )
		);

		// Translators: 1: Date 2: Author 3: Categories 4: Comments
		printf( wp_kses( __( '<div class="header-meta">%1$s%2$s<span class="post-categories">%3$s</span>%4$s</div>', 'brendan' ), array( 
			'div' => array ( 
				'class' => array() ), 
			'span' => array( 
				'class' => array() ) ) ),
 ...
 * @return void
 */
if ( ! function_exists( 'brendan_entry_meta' ) ) {
	function brendan_entry_meta() {
		// Return the Tags as a list
		$tag_list = "";
		if ( get_the_tag_list() ) {
			$tag_list = get_the_tag_list( '<span class="post-tags">', esc_html__( ' ', 'brendan' ), '</span>' );
		}

		// Translators: 1 is tag
		if ( $tag_list ) {
			printf( wp_kses( __( '<i class="fa fa-tag"></i> %1$s', 'brendan' ), array( 'i' => array( 'class' => array() ) ) ), $tag_list );
		}
	}
}
 ...
 * @return void
 */
function brendan_content_width() {
	if ( is_page_template( 'page-templates/full-width.php' ) || is_attachment() ) {
		global $content_width;
		$content_width = 1200;
	}
}
add_action( 'template_redirect', 'brendan_content_width' );
 ...
 */
function brendan_remove_more_jump_link( $link ) {
...
add_filter( 'the_content_more_link', 'brendan_remove_more_jump_link' );
 ...
 */
function brendan_continue_reading_link() {
	return '&hellip;<p><a class="more-link" href="'. esc_url( get_permalink() ) . '" title="' . esc_html__( 'Continue reading', 'brendan' ) . ' &lsquo;' . get_the_title() . '&rsquo;">' . wp_kses( __( 'Continue reading <span class="meta-nav">&rarr;</span>', 'brendan' ), array( 'span' => array( 
			'class' => array() ) ) ) . '</a></p>';
}


/**
 * Replaces "[...]" (appended to automatically generated excerpts) with the brendan_continue_reading_link().
 *
 * @since Brendan 0.0.1
 *
 ...
 */
function brendan_auto_excerpt_more( $more ) {
	return brendan_continue_reading_link();
}
add_filter( 'excerpt_more', 'brendan_auto_excerpt_more' );
 ...
 */
function brendan_new_contactmethods( $contactmethods ) {
	// Add Twitter
...
add_filter( 'user_contactmethods', 'brendan_new_contactmethods', 10, 1 );
 ...
 */
function brendan_add_menu_parent_class( $items ) {
...
add_filter( 'wp_nav_menu_objects', 'brendan_add_menu_parent_class' );
 ...
 */
if ( ! function_exists( 'brendan_get_social_media' ) ) {
	function brendan_get_social_media() {
		$output = '';
		$icons = array(
			array( 'url' => of_get_option( 'social_twitter', '' ), 'icon' => 'fa-twitter', 'title' => esc_html__( 'Follow me on Twitter', 'brendan' ) ),
			array( 'url' => of_get_option( 'social_facebook', '' ), 'icon' => 'fa-facebook', 'title' => esc_html__( 'Like us on Facebook', 'brendan' ) ),
			array( 'url' => of_get_option( 'social_googleplus', '' ), 'icon' => 'fa-google-plus', 'title' => esc_html__( 'Connect with me on Google+', 'brendan' ) ),
			array( 'url' => of_get_option( 'social_linkedin', '' ), 'icon' => 'fa-linkedin', 'title' => esc_html__( 'Connect with me on LinkedIn', 'brendan' ) ),
			array( 'url' => of_get_option( 'social_slideshare', '' ), 'icon' => 'fa-slideshare', 'title' => esc_html__( 'Follow me on SlideShare', 'brendan' ) ),
			array( 'url' => of_get_option( 'social_dribbble', '' ), 'icon' => 'fa-dribbble', 'title' => esc_html__( 'Follow me on Dribbble', 'brendan' ) ),
			array( 'url' => of_get_option( 'social_tumblr', '' ), 'icon' => 'fa-tumblr', 'title' => esc_html__( 'Follow me on Tumblr', 'brendan' ) ),
			array( 'url' => of_get_option( 'social_github', '' ), 'icon' => 'fa-github', 'title' => esc_html__( 'Fork me on GitHub', 'brendan' ) ),
			array( 'url' => of_get_option( 'social_bitbucket', '' ), 'icon' => 'fa-bitbucket', 'title' => esc_html__( 'Fork me on Bitbucket', 'brendan' ) ),
			array( 'url' => of_get_option( 'social_foursquare', '' ), 'icon' => 'fa-foursquare', 'title' => esc_html__( 'Follow me on Foursquare', 'brendan' ) ),
			array( 'url' => of_get_option( 'social_youtube', '' ), 'icon' => 'fa-youtube', 'title' => esc_html__( 'Subscribe to me on YouTube', 'brendan' ) ),
			array( 'url' => of_get_option( 'social_instagram', '' ), 'icon' => 'fa-instagram', 'title' => esc_html__( 'Follow me on Instagram', 'brendan' ) ),
			array( 'url' => of_get_option( 'social_flickr', '' ), 'icon' => 'fa-flickr', 'title' => esc_html__( 'Connect with me on Flickr', 'brendan' ) ),
			array( 'url' => of_get_option( 'social_pinterest', '' ), 'icon' => 'fa-pinterest', 'title' => esc_html__( 'Follow me on Pinterest', 'brendan' ) ),
			array( 'url' => of_get_option( 'social_rss', '' ), 'icon' => 'fa-rss', 'title' => esc_html__( 'Subscribe to my RSS Feed', 'brendan' ) )
		);
 ...
 */
if ( ! function_exists( 'brendan_get_credits' ) ) {
	function brendan_get_credits() {
		$output = '';
		/*
                $output = sprintf( '%1$s <a href="%2$s" title="%3$s">%4$s</a>',
			esc_html__( 'Proudly powered by', 'brendan' ),
			esc_url( esc_html__( 'http://wordpress.org/', 'brendan' ) ),
			esc_attr( esc_html__( 'Semantic Personal Publishing Platform', 'brendan' ) ),
			esc_html__( 'WordPress', 'brendan' )
		);
                 * The code belw needs amending for translation purposes
                 */
                $ocws_theme = wp_get_theme();
                $output = 'This website is powered by <a href="http://wordpress.org">Wordpress</a>, using the <strong>'.$ocws_theme->get( 'Name' ).'</strong> theme, version '.$ocws_theme->get( 'Version' ).' from <a href="http://oldcastleweb.com">Old Castle Web Solutions</a>.';
 ...
 * @return void
 */
function brendan_theme_options_styles() {
	$output = '';
...
add_action( 'wp_head', 'brendan_theme_options_styles' );
 ...
 * @return void
 */
if ( ! function_exists( 'brendan_before_woocommerce_wrapper' ) ) {
	function brendan_before_woocommerce_wrapper() {
 ...
 * @return void
 */
if ( ! function_exists( 'brendan_after_woocommerce_wrapper' ) ) {
	function brendan_after_woocommerce_wrapper() {
 ...
 * @return void
 */
function brendan_is_woocommerce_active() {
 ...
 * @return void
 */
if ( ! function_exists( 'brendan_setup_woocommerce_wrappers' ) ) {
	function brendan_setup_woocommerce_wrappers() {
		if ( brendan_is_woocommerce_active() && is_woocommerce() ) {
				add_action( 'brendan_before_woocommerce', 'brendan_before_woocommerce_wrapper', 10, 0 );
				add_action( 'brendan_after_woocommerce', 'brendan_after_woocommerce_wrapper', 10, 0 );		
		}
	}
	add_action( 'template_redirect', 'brendan_setup_woocommerce_wrappers', 9 );
}
 ...
 * @return void
 */
if ( ! function_exists( 'brendan_woocommerce_before_main_content' ) ) {
	function brendan_woocommerce_before_main_content() {
	...
	add_action( 'woocommerce_before_main_content', 'brendan_woocommerce_before_main_content', 10 );
}
 ...
 * @return void
 */
if ( ! function_exists( 'brendan_woocommerce_after_main_content' ) ) {
	function brendan_woocommerce_after_main_content() {
		echo '</div>';
	}
	add_action( 'woocommerce_after_main_content', 'brendan_woocommerce_after_main_content', 10 );
}
 ...
 * @return void
 */
if ( ! function_exists( 'brendan_remove_woocommerce_sidebar' ) ) {
	function brendan_remove_woocommerce_sidebar() {
		if( ( is_shop() && !of_get_option( 'woocommerce_shopsidebar', '1' ) ) || ( is_product() && !of_get_option( 'woocommerce_productsidebar', '1' ) ) ) {
			remove_action( 'woocommerce_sidebar', 'woocommerce_get_sidebar', 10 );
		}
	}
	add_action( 'woocommerce_before_main_content', 'brendan_remove_woocommerce_sidebar' );
}
 ...
 * @return void
 */
if ( ! function_exists( 'brendan_remove_woocommerce_breadcrumbs' ) ) {
	function brendan_remove_woocommerce_breadcrumbs() {
 ...
 *
 * @since Brendan 0.0.1.1
 *
 * @return void
 */
if ( ! function_exists( 'brendan_set_number_woocommerce_products' ) ) {
	function brendan_set_number_woocommerce_products() {
	...
	add_action( 'init', 'brendan_set_number_woocommerce_products' );
}
